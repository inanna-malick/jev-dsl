...
protoChecks c = do
  let world = state (object ["inquiry" .= ("Where can cancellation drop a computed reply?" :: Text)])
      ...
      edgeOffers = many (.edgeKey) (String . (.edgeText)) edges

  -- tiny use: one question, one answer, no declarations; the row is the payload
  r1 <- ask1 (stub "publication_gate") jevLatest world (choice "Which edge next?" edgeOffers)
  case r1 of
    Left e -> check c ("tiny: " ++ show e) False
    Right a -> do
      checkEq c "tiny: the chosen row is the payload" (Right "publish_if_active")
        (settle routing a (onMany (\_ e -> e.edgeTarget)))
  ...
  case r2 of
    ...
    Right resp -> do
      ...
      checkEq c "packet: a noul is judged under the same policy" (Right True) (judge routing a.enough)
  ...
  let judged p = ask1 (fixed (answerMap [("value", noulAt p)])) jevLatest world (noul "?")
  jy <- judged 0.9
  jn <- judged 0.1
  jd <- judged 0.55
  checkEq c "judge: a clear yes" (Right (Right True)) (fmap (judge merging) jy)
  checkEq c "judge: a clear no is a result, not a doubt" (Right (Right False)) (fmap (judge merging) jn)
  checkEq c "judge: a slim majority still answers under a lenient policy" (Right (Right True)) (fmap (judge routing) jd)
  check c "judge: the middle is a near tie under a policy that wants a margin"
    (case fmap (judge (Policy 0 0.4 0)) jd of
      Right (Left (NearTie ("yes", _) ("no", _))) -> True
      _ -> False)
  ...
  r3 <- ask (stubSplit ["retry_redelivery"] "double_admission") jevLatest world mech
  case r3 of
    Left e -> check c ("wording: " ++ show e) False
    Right resp -> do
      let a = answers resp
  ...
  checkEq c "prepare: two runtime groups colliding" (Just (KeyCollidesWithLabel "value" "k"))
    (prepErr (choice "?" (rows [("k", Null, Edge "e" "" "")] .| many fst (const Null) [("k", Command "c")])))
  ...
  let groups = choice "?" (many fst (String . snd) [("run_retry_fixture" :: Text, "r" :: Text), ("read_publish_gate", "p")])
